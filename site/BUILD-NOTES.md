...
1. **Photos and logo.** The current site blocks image downloads (SiteGround captcha), so the build uses labeled placeholders, and the round scales-of-justice logo is an SVG redrawing. Drop the originals into `site/assets/img/` with these names and rebuild: `logo.png` (then set `LOGO = "logo.png"` in `firm.py`), `couple.jpg`, `headshot-jack.jpg`, `headshot-tara.jpg`, `headshot-cassie.jpg`, `jack-bio.jpg`, `tara-bio.jpg`, `dogs.jpg`, `nest-egg.jpg`, `wealthcounsel.jpg`. The build resizes to 1600 px and generates the social share image from `couple.jpg`.
...
3. **Google review link** (see above). LinkedIn profiles, bar numbers, admission dates and emails for both attorneys are in from the SC Judicial Branch directory and are shown on the attorney pages, the contact page and in the Person schema.
