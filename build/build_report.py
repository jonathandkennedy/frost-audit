...
Search Console exports in ../data. Run: python3 build/build_report.py"""
import csv
import collections
import json
import os
...
frost_q = load_json("frost_queries_2026-06-08_to_2026-09-08.json")["rows"]
frost_qp = load_json("frost_query_page_2026-06-08_to_2026-09-08.json")["rows"]
sva_qp = load_json("sva_query_page_2026-06-08_to_2026-09-08.json")["rows"]
agg = load_json("aggregates.json")
...
def collapse(rows):
    """query+page rows -> per-query dict(clicks, impressions, position, pages)."""
...
# fan-out style questions (main site)
...
# quick wins on the main site's own pages (not the Business Profile link)
qw = {}
for r in frost_qp:
    q, p = r["keys"]
...
# group frost pages by path (duplicate-URL evidence)
...
def table(cols, rows, cls=""):
    """cols: list of (header, key_or_callable, align). rows: dicts."""
...
chart_brand_clicks = C.stacked_bar([("Brand searches (contain “frost”)", B["clicks"], C.BLUE),
                                    ("Non-brand searches", NB["clicks"], C.ORANGE),
                                    ("Searches Google hides for privacy", HID["clicks"], C.GRAY)], aria="Clicks by search type")
chart_brand_impr = C.stacked_bar([("Brand searches (contain “frost”)", B["impressions"], C.BLUE),
                                  ("Non-brand searches", NB["impressions"], C.ORANGE),
                                  ("Searches Google hides for privacy", HID["impressions"], C.GRAY)], aria="Impressions by search type")
...
chart_s_impr = C.line_chart(s_dates, [dict(name="Impressions", values=[r["impressions"] for r in sva_daily], color=C.ORANGE)],
                            annotations=[dict(index=idx(s_dates, "2026-07-27"), text="Jul 27: 190"),
                                         dict(index=idx(s_dates, "2026-09-04"), text="Sep 4: the one click")],
                            legend=False, area=True, aria="Daily impressions for summervilleaccidentattorney.com")
chart_s_pos = C.line_chart(s_dates, [dict(name="Average position", values=[r["position"] for r in sva_daily], color=C.ORANGE)], legend=False,
                           aria="Daily average position for summervilleaccidentattorney.com")
...
chart_s_pr = C.hbar([dict(label=k, value=v["impressions"], note=f"avg. pos {v['avg_position']} · {v['queries']} queries")
                     for k, v in sva_pr.items() if v["impressions"] > 0 and k != "Other"],
                    label_w=250, note_w=170, highlight={"Dog bite", "Motorcycle accident"}, aria="Injury-site impressions by case type")
chart_s_city = C.hbar([dict(label=k, value=v["impressions"], note=f"avg. pos {v['avg_position']} · {v['queries']} queries")
                       for k, v in sva_city.items() if v["impressions"] > 0],
                      label_w=290, note_w=170, highlight={"Goose Creek", "Mt Pleasant", "Moncks Corner", "Walterboro", "North Charleston"},
                      aria="Injury-site impressions by city named in the search")
chart_dead = C.hbar([dict(label=r["path"], value=r["impressions"], note=f"avg. pos {pos(r['position'])}") for r in dead_rows[:12]],
                    label_w=200, note_w=110, color=C.ORANGE, aria="Impressions still landing on removed pages")
chart_overlap = C.dumbbell([(o["query"], o["frost_pos"], o["sva_pos"]) for o in overlap], "frostlawgroupsc.com", "summervilleaccidentattorney.com",
                           label_w=310, aria="The same injury searches show both sites, both far from page 1")
chart_fan = C.hbar([dict(label=q, value=a["impressions"], note=f"avg. pos {pos(a['position'])}") for q, a in fan[:14]],
                   label_w=540, note_w=90, aria="Question-style (fan-out) searches showing the main site")

# diagrams
dia_search = D.flow([
    ("Someone in Summerville searches", "“probate attorney summerville sc”", D.SURF),
    ("Google shows a results page", "AI answer · map pack · 10 links", D.SURF),
    ("Your page is on it", "= 1 impression", C.BLUE),
    ("They click it", "= 1 click", C.NAVY),
    ("They call or book", "= a lead", D.AZALEA),
])
...
dia_estate = D.hub_spoke("Estate Planning hub (/estate-planning-attorney)", [
    "Last will & testament", "Revocable living trust", "Powers of attorney", "Living will",
    ("Asset protection trusts", "new"), ("Estate planning for blended families", "new"), ("Estate plan reviews & trust amendments", "new"),
    ("Estate planning for business owners", "new"), ("Special needs planning", "new"), ("What happens without a will in SC", "new"),
    ("What estate planning costs in SC", "new"), ("Guardianship & conservatorship", "new"),
])
dia_probate = D.hub_spoke("Probate hub (/probate)", [
    "SC probate process (existing section)", ("How long probate takes in SC", "new"), ("Executor duties & disputes", "new"),
    ("Contesting a will in SC", "new"), ("Small-estate / summary administration", "new"), ("Assets that avoid probate", "existing"),
    ("Probate litigation", "existing"), ("Managing inherited assets", "new"), ("Dorchester County Probate Court guide", "new"),
    ("Berkeley County Probate Court guide", "new"), ("Charleston County Probate Court guide", "new"), ("Probate costs & fees in SC", "new"),
], hub_fill=C.NAVY)
dia_crim = D.hub_spoke("Criminal Defense hub (/criminal-defense)", [
    ("Summerville DUI lawyer", "new"), ("First-offense DUI penalties in SC", "new"), ("DUAC & implied consent", "new"),
    ("Drug charges", "new"), ("CDV / domestic violence", "new"), ("Bond hearings in Dorchester County", "new"),
    ("Expungement in SC", "new"), ("Traffic tickets & CDL", "new"), ("Juvenile defense", "new"),
    ("What happens after an arrest in Dorchester County", "new"), ("Summerville Municipal Court guide", "new"), ("Boating under the influence (BUI)", "new"),
], hub_fill=C.NAVY)
dia_pi = D.hub_spoke("Car Accident hub (/practice-areas/car-accidents)", [
    ("Rear-end collisions", "new"), ("Hit-and-run crashes", "new"), ("Uninsured / underinsured motorist", "new"),
    ("Drunk-driving crash victims", "new"), ("Uber & Lyft accidents", "new"), ("Delivery-driver injuries", "new"),
    ("Distracted-driving crashes", "new"), ("I-26 · Hwy 17A · Dorchester Rd crash guide", "new"), ("What to do after a crash in SC", "new"),
    ("Is SC an at-fault state? (comparative negligence)", "new"), ("SC car-accident statute of limitations", "new"), ("How settlements work in SC", "new"),
], hub_fill=C.ORANGE, accent=C.ORANGE)
dia_pi2 = D.hub_spoke("Injury site: other hubs", [
    "Truck accidents", "Motorcycle accidents", "Dog bites (SC strict liability)", "Wrongful death", "Slip & fall", "Workers’ comp",
    ("Goose Creek", "new"), ("Ladson · Knightsville", "new"), ("Moncks Corner", "new"), ("North Charleston · West Ashley", "new"),
    ("Mount Pleasant · Charleston", "new"), ("Abogado de accidentes (Spanish)", "new"),
], hub_fill=C.ORANGE, accent=C.ORANGE)
dia_cycle = D.cycle([
    ("1 answer page", "on the right site", C.NAVY),
    ("1 YouTube video", "the question is the title", D.SURF),
    ("3 vertical clips", "YouTube Shorts + Reels", D.SURF),
    ("1 LinkedIn post", "Tara or Jack, in their voice", D.SURF),
    ("1 Instagram reel + carousel", "the pups can co-star", D.SURF),
    ("1 Business Profile post", "with the link", D.SURF),
    ("FAQ + video schema", "so Google can read it", D.SURF),
])
dia_gantt = D.gantt([
    ("## Weeks 1–2 · plumbing", 0, 0, "", ""),
    ("One address per page (redirects, canonicals)", 1, 2, C.NAVY, ""),
    ("Unique title tags on every page (both sites)", 1, 2, C.NAVY, ""),
    ("Injury site: 17 redirects for removed pages", 1, 2, C.NAVY, ""),
    ("Business Profile: URL, categories, photos", 1, 2, C.NAVY, ""),
    ("Request re-indexing of home + money pages", 2, 2, C.NAVY, ""),
    ("## Weeks 3–6 · money pages", 0, 0, "", ""),
    ("DUI · drug · CDV · guardianship · asset protection", 3, 6, C.BLUE, ""),
    ("Injury: Goose Creek + 4 city pages + Spanish", 3, 6, C.ORANGE, ""),
    ("Retitle & expand existing practice pages", 3, 5, C.BLUE, ""),
    ("Yelp: claim, categories, photos, specialties", 3, 4, C.AQUA, ""),
    ("## Weeks 5–12 · answer pages & channels", 0, 0, "", ""),
    ("2 answer pages / week (main site)", 5, 12, C.BLUE, ""),
    ("1 answer page / week (injury site)", 5, 12, C.ORANGE, ""),
    ("YouTube launch: 1 video / week", 5, 12, C.AQUA, ""),
    ("LinkedIn + Instagram cadence", 5, 12, C.AQUA, ""),
    ("Weekly Business Profile post + review asks", 3, 12, C.AQUA, ""),
    ("Monthly Search Console scorecard", 4, 12, C.GRAY, ""),
], label_w=330)
...
<meta name="description" content="Search Console audit and topical-authority plan for frostlawgroupsc.com and summervilleaccidentattorney.com">
<style>{CSS}</style>
<div class="sheet">
<header class="cover">
<div class="eyebrow">Search audit &amp; topical-authority plan · September 2026</div>
<h1>Frost Law Group Search Audit</h1>
<p class="lead" style="margin-top:14px">Two websites, one plan. What Google Search Console says about <strong>frostlawgroupsc.com</strong> and <strong>summervilleaccidentattorney.com</strong>, what it means in plain English, and exactly what to build next — on the sites, on YouTube, on LinkedIn, on Instagram, on your Google Business Profile and on Yelp.</p>
<div class="meta">
<div><b>Prepared for</b>Tara &amp; Jack Frost · Frost Law Group, LLC · Summerville, SC</div>
<div><b>Data source</b>Google Search Console (the firm's own properties)</div>
<div><b>Main site window</b>{F_FIRST.strftime('%b %-d')} – {F_LAST.strftime('%b %-d, %Y')} ({F_DAYS} days of data)</div>
<div><b>Injury site window</b>{S_FIRST.strftime('%b %-d')} – {S_LAST.strftime('%b %-d, %Y')} ({S_DAYS} days of data)</div>
<div><b>Report date</b>September 11, 2026</div>
</div>
<p class="small" style="margin-top:22px">Every number in this report comes from Google Search Console for the two properties above. Where we describe the live pages or the Business Profile, we say so. Search Console's last two or three days are provisional and are shown as reported.</p>
</header>
...
add(sec("summary", "1 · Read this first", "The 60-second version",
        "Both websites are being <em>shown</em> by Google far more than they are being <em>clicked</em>. The main site is found by people who already know your name; the injury site is on page 5–6 for everything. The good news: Google already treats the firm as relevant for estate, probate, criminal and DUI searches in Summerville — the pages just aren't built to win the click yet."))
add("<h3>frostlawgroupsc.com</h3>")
add(kpis([("Clicks", fmt(F["clicks"]), f"{F_DAYS} days"), ("Impressions", fmt(F["impressions"]), "times a page was shown"),
          ("Click-through rate", pct(F["ctr"]), "clicks ÷ impressions"), ("Average position", pos(F["position"]), "1 = top of page 1"),
          ("Non-brand clicks", fmt(NB["clicks"]), f"from {fmt(NB['impressions'])} non-brand impressions")]))
add("<h3>summervilleaccidentattorney.com</h3>")
add(kpis([("Clicks", fmt(S["clicks"]), f"{S_DAYS} days"), ("Impressions", fmt(S["impressions"]), "times a page was shown"),
          ("Click-through rate", pct(S["ctr"], 2), "clicks ÷ impressions"), ("Average position", pos(S["position"]), "page 6 of results"),
          ("Queries on page 1", fmt(sh["1-3"]["queries"] + sh["4-10"]["queries"]), f"of {fmt(sum(v['queries'] for v in sh.values()))} queries seen")]))
...
add("<h3>Six findings that explain the numbers</h3>")
...
add(todo([
    ...
    "<strong>Tune the Business Profile.</strong> Categories, photos, services, a weekly post, and a review routine. It is already the thing Google shows most — make it convert.",
...
add(plain("Google is a librarian. Right now she pulls your book off the shelf a lot, but the cover is blank (same title on every page), the book is filed under two call numbers (two addresses), and it isn't the book people asked for (they asked about DUI; you handed them “Criminal Defense”). We're going to give every page its own cover, one call number, and write the exact books people are asking for."))
add("</section>")

# ---------------------------------------------------------------- how it works
add(sec("how", "2 · The basics", "How Google works, explained simply",
        "Everything in this report uses five words. Here is what they mean, with the picture that goes with them."))
...
add("<p>When someone asks Google's AI a big question — “who should handle my dad's estate in Summerville?” — the AI doesn't search that sentence. It quietly splits it into five or ten smaller questions, searches each one, and stitches the answers together. Those smaller questions are called fan-out queries. They show up in Search Console as oddly precise, complete sentences. You already have " + f"{FAN_N} of them. Section 8 shows how to use them.</p>")
...
add(fig(chart_f_clicks, "Daily clicks", "Never more than nine in a day; most days one to three",
        "Clicks are flat because nearly all of them are people searching your name. The non-brand line above is what has to grow."))
...
add(table([("Page", lambda g: f"<code>{esc(g['path'])}</code>" + (f' <span class="pill warn">{len(g["variants"])} addresses</span>' if len(g["variants"]) > 1 else ""), ""),
           ("Clicks", lambda g: fmt(g["clicks"]), "n"), ("Impressions", lambda g: fmt(g["impressions"]), "n"),
           ("Avg. position", lambda g: pos(g["position"]), "n"),
           ("Addresses Google is counting separately", lambda g: "<br>".join(f"<code>{esc(v)}</code>" for v in g["variants"]), "")],
          page_groups))
...
add(table([("Topic", "k", ""), ("Queries", lambda r: fmt(r["v"]["queries"]), "n"), ("Impressions", lambda r: fmt(r["v"]["impressions"]), "n"),
           ("Clicks", lambda r: fmt(r["v"]["clicks"]), "n"), ("Avg. position", lambda r: r["v"]["avg_position"], "n"),
          ...
          [dict(k=k, v=v) for k, v in topics.items() if v["impressions"] > 0]))
...
add(table([("Search", lambda r: esc(r["query"]), ""), ("Impr.", lambda r: fmt(r["impressions"]), "n"), ("Pos.", lambda r: pos(r["position"]), "n"),
           ("Page today", lambda r: f"<code>{esc(re.sub(r'^https://frostlawgroupsc.com', '', r['page']))}</code>", ""), ("What to do", lambda r: action_for(r["query"]), "")],
          quick_frost[:24]))
add("<h3>3.7 The questions Google's AI is already asking you</h3>")
add(fig(chart_fan, f"Question-style searches (fan-out) that showed the main site", f"{FAN_N} questions · {fmt(FAN_I)} impressions · positions mostly 4–13",
        "Notice the shape: complete, specific, local sentences. Nobody types these. Google's AI generates them when a person asks a bigger question, and it is pulling your estate-planning, probate and trust pages into the answer. Every one of these deserves its own short page (Section 8)."))
...
tech1 = [
    ...
    ("warn", "The personal-injury page competes with the injury site.", f"<code>/motor-vehicle-personal-injury</code> (two addresses, {fmt(sum(r['impressions'] for r in frost_pages if 'motor-vehicle' in r['url']))} impressions, average position 14–32) targets the same searches as summervilleaccidentattorney.com. Section 5 gives the rule; the short version is: one site per subject."),
...
add(fig(chart_s_impr, "Daily impressions", f"{S_FIRST.strftime('%b %-d')} – {S_LAST.strftime('%b %-d, %Y')}",
        "Impressions fell through August as the removed old-site pages started dropping out of Google's index, then recovered slightly in September as the new practice-area pages were picked up."))
add(fig(chart_s_pos, "Daily average position", "Lower is better; page one is 1–10",
        "The site has never averaged better than position 30 on any day. Page-six positions are effectively invisible: fewer than one searcher in a hundred looks there."))
...
add(plain("Google knows this site exists and files it under the right subject. It just doesn't trust it yet: the site is new, has almost no reputation, and lost its most-seen pages in the rebuild. Trust is built with the plan in Sections 6–9; the redirects in 4.4 stop the bleeding first."))
...
add(table([("Old address", lambda r: f"<code>{esc(r['path'])}</code>", ""), ("Impressions", lambda r: fmt(r["impressions"]), "n"), ("Avg. pos.", lambda r: pos(r["position"]), "n"),
           ("Redirect to", lambda r: esc(r["to"]), "")], dead_rows))
...
add(table([("Search", lambda r: esc(r[0]), ""), ("Impr.", lambda r: fmt(r[1]["impressions"]), "n"), ("Avg. pos.", lambda r: pos(r[1]["position"]), "n"),
           ("What to do", lambda r: ("Expand the dog-bite page: SC strict-liability statute, children, homeowner's insurance, scarring; add a dog-bite video and FAQ schema." if "dog bite" in r[0] else
                                     ...
                                     "Add a premises-liability section to the slip-and-fall page." if "premises" in r[0] else "Answer page + FAQ on the closest practice page."), "")],
          quick_sva[:16]))
...
# ---------------------------------------------------------------- two sites
add(sec("twosites", "5 · The decision", "Two websites: the honest pros and cons",
        "You asked for a straight answer on whether to keep both sites. Here is the evidence, the trade-offs in plain language, and the rules that make two sites work if that's the choice."))
...
add('<div class="card con"><h4>What it costs you</h4><ul>'
    '<li><strong>Reputation splits.</strong> Every link, mention and review builds one domain. The injury site starts from almost nothing and currently averages position ' + pos(S["position"]) + '.</li>'
...
add("<p><strong>Keep both — but only under the seven rules below, and with a checkpoint.</strong> The data supports the split: the main site's strength is estate, probate and criminal (that is where Google already trusts it), and the injury market in Summerville is dominated by large firms with big review counts, which a focused, family-run injury site can position against better than a general practice page can. The risks are all manageable if the sites are wired correctly from day one.</p>")
...
# ---------------------------------------------------------------- authority plan
add(sec("authority", "6 · The plan", "The topical-authority plan: write the shelf",
        "Four hubs on the main site, two on the injury site. Each hub is an existing practice page rebuilt as the center of a cluster; each spoke is a page that answers one question people ask. Blue outlines are pages to build."))
add("<h3>6.1 Main site clusters</h3>")
add(fig(dia_estate, "Estate Planning cluster", "Hub: the existing estate-planning page · 12 spokes, 8 new",
        "The existing will, trust, POA and living-will pages become spokes with proper titles. New spokes come straight from Search Console: asset protection (six different searches), blended families, trust amendments and restatements, guardianship, costs, and “what happens without a will”."))
add(fig(dia_probate, "Probate cluster", "Hub: the existing probate page · 12 spokes, 9 new",
        "Probate is where Google's AI already quotes you. The county court guides (Dorchester, Berkeley, Charleston) are the kind of useful, local page nobody else in Summerville has written — and they attract links from local organizations."))
add(fig(dia_crim, "Criminal Defense cluster", "Hub: the existing criminal-defense page · 12 spokes, all new",
        "DUI is the biggest gap: 24 DUI searches, 410 impressions, no DUI page. Jack's fourteen years in law enforcement is the story every page in this cluster should tell — “a former deputy, now on your side” is a real, defensible difference."))
add("<h3>6.2 Injury site clusters</h3>")
add(fig(dia_pi, "Car Accident cluster", "Hub: the existing car-accident page · 12 spokes, all new",
        "Search Console already shows demand for hit-and-run, drunk-driving-victim, uninsured-motorist, distracted-driving, rideshare and delivery-driver searches — almost all of them with “Goose Creek” or “Summerville” attached."))
add(fig(dia_pi2, "The other injury hubs and the city pages", "Existing practice pages plus six city pages and a Spanish page",
        "City pages are only worth building if they are genuinely local: the roads and intersections, the hospital people get taken to, the police department that writes the report, the courthouse, directions from that town to the office."))
add("<h3>6.3 How every page is built</h3>")
add('<div class="grid2">')
...
add(todo([
    "Every page links to its hub; every hub links to every spoke; no page is more than two clicks from the home page.",
    "One page per question. If two pages could rank for the same search, merge them.",
    "Structured data on every page: LegalService (organization), Attorney (bios), FAQPage (answer blocks), VideoObject (embedded videos), BreadcrumbList.",
    "Real photos, real names, real courts. Google's quality guidelines for legal content weight who wrote it and whether they are who they say they are.",
    "Publish, then request indexing in Search Console; check impressions on the new page at 14 and 45 days.",
]))
add("</section>")

# ---------------------------------------------------------------- keywords
add(sec("keywords", "7 · Keywords", "High intent, long tail, and where to win first",
        "“High intent” means the person is ready to hire (“dui lawyer summerville sc”). “Long tail” means specific, longer questions with fewer searches each but far less competition — and they add up. We rank both by how close you already are, using your own Search Console positions."))
add("<h3>7.1 Main site: high-intent targets</h3>")
hi_main = [
    ("Summerville DUI lawyer", "summerville dui lawyer · dui lawyer summerville sc · dui attorney summerville (sc) · summerville dui attorney · drinking and driving attorney summerville sc", "new /dui-lawyer-summerville-sc", "Profile shown at #1; criminal page at ~30", "1"),
    ("Criminal defense attorney Summerville SC", "criminal defense attorney summerville sc · summerville criminal defense attorney/lawyer · criminal defense lawyer summerville sc · criminal attorney summerville sc", "/criminal-defense (retitle + expand)", "25–35", "1"),
    ("Estate planning attorney Summerville SC", "estate planning attorney summerville sc · summerville estate planning attorney · estate lawyer summerville sc · summerville estate attorney · estate planning attorney in ladson sc", "/estate-planning-attorney (retitle + expand)", "8–17", "1"),
    ("Probate lawyer Summerville SC", "probate lawyer summerville sc · probate attorney summerville · summerville probate · summerville probate litigation lawyer · avoiding probate summerville", "/probate (retitle + executor section)", "5–17", "1"),
    ("Wills lawyer Summerville SC", "wills lawyer summerville sc · summerville wills · wills and estate planning lawyer summerville sc · summerville wills and trusts", "/last-will-and-testament (retitle)", "4–11", "1"),
    ("Living trust attorney Summerville", "living trust lawyers summerville · summerville revocable living trusts · trusts summerville · creating a trust summerville · summerville trust attorneys", "/revocable-trust (retitle)", "8–12", "2"),
    ("Guardianship & conservatorship lawyer Summerville", "summerville conservatorship lawyer · which attorneys in summerville, sc handle contested guardianship cases? · guardianship of minors", "new /guardianship-conservatorship", "5–12 (no page!)", "1"),
    ("Asset protection attorney Summerville", "asset protection attorneys/lawyers summerville · asset protection planning summerville · who helps create asset protection trusts in summerville, sc?", "new /asset-protection", "9–17", "2"),
    ("Drug charge lawyer Summerville SC", "drug defense attorney summerville sc · drug lawyer summerville sc · drug defense lawyer summerville sc", "new /drug-charges", "5–29", "2"),
    ("CDV / domestic violence attorney Summerville", "cdv attorney summerville sc · domestic violence lawyer", "new /domestic-violence-cdv", "24", "2"),
    ("Power of attorney lawyer Summerville", "summerville durable power of attorneys · durable power of attorneys summerville · power of attorney lawyer south carolina", "/power-of-attorney (retitle)", "6–18", "2"),
    ("Living will attorney Summerville", "living will sc · sc living will form · living will south carolina", "/living-will (retitle + SC form explainer)", "40–56", "3"),
]
add(table([("Target", lambda r: f"<strong>{r[0]}</strong>", ""), ("Searches already seen in Search Console", lambda r: r[1], ""), ("Page", lambda r: f"<code>{r[2]}</code>", ""), ("Position today", lambda r: r[3], "n"), ("Priority", lambda r: pill("info", "P" + r[4]), "")], hi_main))
add("<h3>7.2 Main site: long-tail and question targets</h3>")
add("<p>The first group is verbatim from Search Console (with impressions and position). The second group is the logical next layer of the same clusters — questions Google's AI will fan out to once the hubs exist. No third-party numbers are used; the test for each is “is this the exact question a Summerville family asks?”</p>")
add(table([("Question already showing the site", lambda r: esc(r[0]), ""), ("Impr.", lambda r: fmt(r[1]["impressions"]), "n"), ("Pos.", lambda r: pos(r[1]["position"]), "n"),
           ("Page to write", lambda r: action_for(r[0]).replace("New answer page: ", "").replace("New page: ", ""), "")], fan[:18]))
add("<h4>Next-layer questions (build after the hubs)</h4>")
add('<ul class="two">' + "".join(f"<li>{q}</li>" for q in [
    "How long does probate take in South Carolina?", "How much does probate cost in SC, and who pays?", "Does a will have to be probated in South Carolina?",
    "What happens if you die without a will in South Carolina?", "How much does an estate plan cost in Summerville?", "Will vs. revocable trust in South Carolina — which do we need?",
    "South Carolina power of attorney requirements (witnesses, notary, recording)", "How do I remove an executor in South Carolina?", "Can I contest a will in Dorchester County?",
    "What does a healthcare power of attorney cover in SC?", "How do we fund a revocable trust (deeds, accounts, vehicles)?", "Estate planning for a second marriage in SC",
    "First-offense DUI penalties in South Carolina", "What is DUAC and how is it different from DUI?", "Do I have to take the breathalyzer in SC (implied consent)?",
    "How does a bond hearing work in Dorchester County?", "Can a CDV charge be expunged in South Carolina?", "What happens after an arrest in Summerville (first 72 hours)?",
    "How to get a criminal record expunged in SC", "Do I need a lawyer for a first-offense drug charge in SC?", "What is a conservatorship and when does a family need one?",
]) + "</ul>")
add("<h3>7.3 Injury site: high-intent targets</h3>")
hi_pi = [
    ("Summerville dog bite lawyer", "dog bite lawyer summerville sc · summerville dog bite lawyer/attorney(s) · dog bite law firm summerville · dog bite lawyer knightsville", "/practice-areas/dog-bites (expand)", "20–27", "1"),
    ("Summerville motorcycle accident lawyer", "motorcycle accident lawyer summerville (sc) · summerville motorcycle accident attorney/lawyer · summerville motorcycle crash lawyer", "/practice-areas/motorcycle-accidents (+ redirect)", "17–36", "1"),
    ("Summerville truck accident lawyer", "summerville truck accident lawyer/attorney(s) · truck accident lawyer summerville · summerville fatal truck accident lawyer", "/practice-areas/truck-accidents (+ redirect)", "30–66", "1"),
    ("Goose Creek car accident lawyer", "goose creek car accident lawyer/attorney · goose creek hit and run/drunk driving/distracted driving/uninsured motorist accident attorney (40 searches)", "new /areas/goose-creek", "49–75 (page removed)", "1"),
    ("Summerville car accident lawyer", "summerville car accident lawyer/attorney(s) · car accident lawyer summerville sc · summerville auto accident attorney/lawyer · summerville car accident law firm", "/practice-areas/car-accidents (retitle + rebuild)", "44–89", "1"),
    ("Abogado de accidentes en Summerville", "abogado(s) de accidentes carro summerville · abogado de accidentes coche summerville · abogado de accidentes carro goose creek", "new /es/abogado-de-accidentes", f"29–71 ({fmt(SPAN_I)} impr.)", "2"),
    ("Summerville Uber / Lyft accident lawyer", "summerville uber accident attorney/lawyer(s) · summerville lyft accident attorney/lawyer(s) · goose creek uber accident attorney", "new /practice-areas/rideshare-accidents", "48–82", "2"),
    ("Drunk-driving crash victims", "summerville drunk driving accident attorney/lawyer(s) · summerville dui accident attorney · goose creek drunk driving accident attorney", "new /practice-areas/drunk-driving-accidents", "59–84", "2"),
    ("Mount Pleasant / North Charleston / Moncks Corner / Walterboro", "mount pleasant car accident lawyer(s) · mt pleasant car accident attorney · north charleston car accident attorney · moncks corner car accident attorneys · walterboro car accident lawyer", "new /areas/… pages", "13–66 (pages removed)", "2"),
    ("Summerville wrongful death lawyer", "summerville wrongful death attorney/lawyer · summerville fatal car accident attorney/lawyer(s) · wrongful death law firm summerville", "/practice-areas/wrongful-death (+ redirect)", "49–80", "3"),
    ("Delivery-driver & work injuries", "summerville delivery driver injury attorney/lawyer(s) · summerville mechanic injury lawyer · summerville work injury lawyer", "new spoke under car accidents + workers' comp", "67–90", "3"),
]
add(table([("Target", lambda r: f"<strong>{r[0]}</strong>", ""), ("Searches already seen in Search Console", lambda r: r[1], ""), ("Page", lambda r: f"<code>{r[2]}</code>", ""), ("Position today", lambda r: r[3], "n"), ("Priority", lambda r: pill("info", "P" + r[4]), "")], hi_pi))
add("<h4>Injury site: long-tail and question targets</h4>")
add('<ul class="two">' + "".join(f"<li>{q}</li>" for q in [
    "What to do after a car accident in South Carolina (the first 48 hours)", "Is South Carolina an at-fault state?", "How does comparative negligence work in SC (the 51% rule)?",
    "How long do I have to file a car-accident claim in SC (statute of limitations)?", "How much is a car accident settlement in South Carolina?", "Should I talk to the other driver's insurance company?",
    "What is South Carolina's dog-bite law (strict liability)?", "What if my child was bitten by a neighbor's dog?", "Does homeowner's insurance pay for a dog bite in SC?",
    "Who pays after an Uber or Lyft accident in South Carolina?", "What if the driver who hit me was drunk — can I get punitive damages?", "What if the driver who hit me has no insurance (UM/UIM in SC)?",
    "Motorcycle helmet law in South Carolina and your claim", "Where do crashes happen on I-26, Hwy 17A and Dorchester Road?", "Getting your injuries evaluated after a crash in Summerville (the “auto injury assessment” search)",
    "Accident on the Highway 17 bypass — who is responsible?",
]) + "</ul>")
add("<h3>7.4 The “win fast” order</h3>")
...
add("<ol>"
    ...
    "<li><strong>Main site, week 3–4:</strong> the DUI page and the guardianship page — both have demand with no page, and the profile already ranks #1 for DUI.</li>"
    "<li><strong>Main site, week 3–6:</strong> executor-disputes, trust-amendment, asset-protection and blended-family answer pages — the fan-out questions already at positions 4–10.</li>"
    "<li><strong>Injury site, week 1–2:</strong> the 17 redirects and the www fix.</li>"
    "<li><strong>Injury site, week 3–6:</strong> expand dog bites (closest to page one), rebuild Goose Creek, retitle motorcycle and truck, add the Spanish page.</li>"
    "</ol>")
add("</section>")

# ---------------------------------------------------------------- fan-out
add(sec("fanout", "8 · AI search", "Using Google's fan-out queries to our advantage",
        "Fan-out is the biggest change in how Google works in years, and your Search Console already shows it happening. Here is the playbook."))
add("<h3>8.1 Harvest the questions every month</h3>")
...
add("<h3>8.2 Answer each one on its own page, answer-first</h3>")
...
add("<h3>8.3 Be the same firm everywhere</h3>")
...
# ---------------------------------------------------------------- channels
add(sec("channels", "9 · Channels", "YouTube, LinkedIn, Instagram, the Business Profile and Yelp",
        "The websites are where authority lives; the channels are how it spreads and how Google learns the firm is real, active and trusted. One piece of work feeds all of them."))
add(fig(dia_cycle, "The content flywheel", "One answer page a week becomes seven assets"))
channels = [
    ("YouTube", "Google owns it, indexes every transcript, and shows videos in results and AI answers. There is no Frost Law Group channel today.",
     ["Create the channel as the firm; link it from both sites, the Business Profile and LinkedIn.",
      "Series 1 — <em>Ask a Summerville Lawyer</em>: 60–90 second answers to the fan-out questions (vertical, Shorts).",
      "Series 2 — <em>Probate in Plain English</em> (Tara, 4–8 minutes): the probate process, executor duties, timelines, costs — one video per spoke page.",
      "Series 3 — <em>Know Your Rights in SC</em> (Jack, 3–6 minutes): DUI stops, bond hearings, what to say and not say — the former-deputy angle.",
      "Series 4 — <em>After the Crash</em> (injury site): what to do, insurance calls, dog bites, motorcycle bias.",
      "Title = the question. Description = the 60-word answer + link to the page. Chapters, captions and a pinned comment with the phone number.",
      "Embed every video on its page and mark it up as VideoObject."],
    ...
    ("LinkedIn", "Where estate-planning referrals come from: financial advisors, CPAs, realtors, HR managers. Tara's and Jack's personal profiles reach more people than the company page.",
     ["Tara posts weekly on estate and probate for professionals: “What a financial advisor should check before a client funds a trust”, “Three probate mistakes we see in Dorchester County”.",
      "Jack posts weekly on criminal and injury topics for the community: bond hearings, what a police report means, the drunk-driving-victim path.",
      "Company page shares every new answer page and video; website field → main site; injury posts link to the injury site.",
      "Ask five referral partners a month to follow and comment; comment on theirs. Send a quarterly “what changed in SC estate law” post to the same list.",
      "Add both attorneys' LinkedIn URLs to the Attorney structured data on both sites."],
    ...
    ("Instagram", "Your existing account (@frostlawgroupllc) is the human side: the husband-and-wife firm, the Golden Retrievers, Summerville. It builds brand recall and sends Google engagement and entity signals.",
     ["Reels: the same vertical clips as YouTube Shorts (the question as on-screen text in the first second).",
      "Carousels: “5 things to do after a dog bite in SC”, “What a will does and doesn't do”, “DUI stop: your 4 rights”.",
      "The Frost Pups as a recurring feature — office life, Flowertown Festival, local events; geo-tag Summerville on everything.",
      "Bio: a link page with both websites, the Business Profile review link and the phone number.",
      "Highlights: Estate · Probate · Criminal · Injury · The Pups."],
     "3 posts per week: one reel, one carousel, one photo."),
    ("Google Business Profile", f"It already produces more impressions than any page ({fmt(home_www['impressions'])} in {F_DAYS} days). Right now it is a plain listing: the “Attorney” category plus “Estate planning attorney”, a handful of photos, no posts.",
     ["Website: change to <code>https://frostlawgroupsc.com/</code> (it currently stores <code>http://www.</code>).",
      "Categories: primary <strong>Estate planning attorney</strong>; add Criminal justice attorney, Personal injury attorney, Probate attorney, Elder law attorney, Trial attorney, Law firm.",
      "Photos: 20+ real photos (exterior with the sign, reception, conference room, Jack, Tara, Cassie, the dogs) — then two new photos a month.",
      "Services: keep the list, add a two-sentence description with the local phrase for each (“Summerville DUI defense…”).",
      "Posts: one a week, reusing the answer page and video; every post has a call to action.",
      "Q&amp;A: seed eight questions (the fan-out questions) with the firm's answers.",
      "Reviews: ask at every closed matter with a direct link; goal four to six new reviews a month; reply to every review, including the one-star.",
      "Verify ownership: the public listing shows an “Own this business?” prompt, which Google typically displays when a profile isn't verified — confirm in the Business Profile dashboard.",
      "Hours: consider a 24/7 intake line for injury calls (an answering service) so the injury practice can honestly show extended availability."],
     "Weekly post, monthly photos, review ask at every matter."),
    ("Yelp", "ChatGPT, Bing and Apple Maps use Yelp listings and reviews when someone asks them for a lawyer nearby. The listing exists (category: Criminal Defense Law) and is thin.",
     ["Claim/verify and complete: categories Estate Planning Law, Wills Trusts &amp; Probates, Criminal Defense Law, DUI Law, Personal Injury Law.",
      "“From the business” and specialties written as short answers to the fan-out questions.",
      "Ten real photos; hours; website → main site; phone.",
      "Respond to every review. Don't mass-solicit (Yelp filters that); do mention the Yelp page in the review email as a second option for clients who use Yelp.",
      "Update it quarterly so it stays “Updated [month]” in Google's results."],
...
# ---------------------------------------------------------------- plan
add(sec("plan", "10 · Timeline", "The 90-day plan, and the year after",
        "Plumbing first, money pages second, then the weekly rhythm of answer pages and video. Nothing here depends on buying anything; it depends on Jack's and Tara's time in front of a camera and a partner who builds the pages."))
...
add(todo([
    "County court guides (Dorchester, Berkeley, Charleston probate courts; Summerville Municipal Court) — the pages local organizations link to.",
    "Local links: Summerville Chamber (already lists the firm), Dorchester County Bar, WealthCounsel directory, sponsorships (youth sports, Flowertown Festival), local news quotes. Update Avvo, Justia, FindLaw and Super Lawyers profiles to list both sites.",
    "Second layer of answer pages for each cluster (the “next-layer questions” in Section 7).",
    "Review milestone: 50+ Google reviews, with practice-area keywords appearing naturally in them.",
    "Month-six checkpoint on the injury site (Section 5).",
]))
add("<h3>Months 7–12: expansion</h3>")
add(todo([
    "Injury site: catastrophic-injury and workers'-comp clusters; case-results page; more city pages only where searches appear.",
...
add(plain("First we fix the plumbing so nothing leaks. Then we build the rooms people are asking for. Then, every week, we add one more book to the shelf and tell people about it in five places. Twelve months of that is how a two-person firm becomes the name Google says first."))
add("</section>")

# ---------------------------------------------------------------- measurement
add(sec("measure", "11 · Measurement", "How we'll know it's working",
        "Five numbers from Search Console, checked monthly, plus the only number that really matters: calls and consultations."))
add(table([("Signal", "s", ""), ("Today", "t", "n"), ("What good looks like", "g", ""), ("Where to see it", "w", "")], [
    dict(s="Non-brand clicks per month (main site)", t=f"≈{fmt(round(NB['clicks'] / F_DAYS * 30))}", g="Growing every month; the first goal is double digits every month, then dozens", w="Performance → Queries, filter “does not contain frost”"),
    ...
    dict(s="Question-style (fan-out) searches showing the site", t=f"{FAN_N} questions · {fmt(FAN_I)} impressions", g="More questions each month, average position moving toward 1–3", w="Queries filtered by the regular expression in Section 8"),
    dict(s="Injury site: queries on page one", t="0", g="Dog bite, motorcycle and Goose Creek searches on page one by month six", w="Injury property → Performance"),
    dict(s="Injury site: non-brand clicks per month", t=f"{fmt(S['clicks'])} in {S_DAYS} days", g="First weekly clicks by month three; the month-six checkpoint", w="Injury property → Performance"),
...
# ---------------------------------------------------------------- appendix
add(sec("appendix", "12 · Appendix", "Data tables, redirects, title tags, glossary"))
...
add(table([("Search", lambda r: esc(r[0]), ""), ("Impressions", lambda r: fmt(r[1]["impressions"]), "n"), ("Avg. position", lambda r: pos(r[1]["position"]), "n"),
           ("Pages shown", lambda r: ", ".join(f"<code>{esc(re.sub(r'^https?://(www.)?summervilleaccidentattorney.com', '', p) or '/')}</code>" for p in list(r[1]["pages"])[:2]), "")], top_sva))
...
add(table([("Page", lambda r: f"<code>{esc(r['url'])}</code>", ""), ("Clicks", lambda r: fmt(r["clicks"]), "n"), ("Impressions", lambda r: fmt(r["impressions"]), "n"), ("Avg. position", lambda r: pos(r["position"]), "n"),
           ("Status", lambda r: (pill("crit", "removed · redirect") if re.sub(r"^https?://(www\.)?summervilleaccidentattorney\.com", "", r["url"]) in DEAD else (pill("warn", "non-www duplicate") if r["url"].startswith("https://summervilleaccidentattorney.com/") else pill("good", "live"))), "")],
          sorted(sva_pages, key=lambda r: -r["impressions"])))
add("<h3>C. Proposed title tags</h3>")
titles_main = [
    ("/", "Summerville, SC Estate Planning, Probate & Criminal Defense Attorneys | Frost Law Group"),
    ("/estate-planning-attorney", "Estate Planning Attorney in Summerville, SC | Wills, Trusts & POAs | Frost Law Group"),
    ("/probate", "Probate Attorney in Summerville, SC | Dorchester & Berkeley County Estates | Frost Law Group"),
    ("/criminal-defense", "Criminal Defense Attorney in Summerville, SC | Former Deputy Jack Frost | Frost Law Group"),
    ("/dui-lawyer-summerville-sc (new)", "Summerville DUI Lawyer | First-Offense DUI & DUAC Defense | Frost Law Group"),
    ("/drug-charges (new)", "Drug Charge Defense Lawyer in Summerville, SC | Frost Law Group"),
    ("/domestic-violence-cdv (new)", "CDV / Domestic Violence Defense Attorney in Summerville, SC | Frost Law Group"),
    ("/guardianship-conservatorship (new)", "Guardianship & Conservatorship Attorney in Summerville, SC | Frost Law Group"),
    ("/asset-protection (new)", "Asset Protection Trust Attorney in Summerville, SC | Frost Law Group"),
    ("/last-will-and-testament", "Wills Lawyer in Summerville, SC | Last Will & Testament | Frost Law Group"),
    ("/revocable-trust", "Revocable Living Trust Attorney in Summerville, SC | Frost Law Group"),
    ("/power-of-attorney", "Power of Attorney Lawyer in Summerville, SC | Durable & Healthcare POA | Frost Law Group"),
    ("/living-will", "Living Will Attorney in Summerville, SC | Frost Law Group"),
    ("/about-us", "About Jack & Tara Frost | Summerville, SC Husband-and-Wife Law Firm | Frost Law Group"),
    ("/contact-us", "Contact Frost Law Group | Summerville, SC Attorneys | (843) 419-6653"),
    ("/blog", "In The Know: Estate, Probate & Criminal Law Guides for the Lowcountry | Frost Law Group"),
]
titles_pi = [
    ("/", "Summerville Car Accident & Personal Injury Lawyers | Frost Law Group"),
    ("/practice-areas", "Personal Injury Practice Areas | Summerville, SC | Frost Law Group"),
    ("/practice-areas/car-accidents", "Summerville Car Accident Lawyer | Free Consultation | Frost Law Group"),
    ("/practice-areas/truck-accidents", "Summerville Truck Accident Lawyer | 18-Wheeler Crash Attorneys | Frost Law Group"),
    ("/practice-areas/motorcycle-accidents", "Summerville Motorcycle Accident Lawyer | Frost Law Group"),
    ("/practice-areas/dog-bites", "Summerville Dog Bite Lawyer | SC Strict-Liability Claims | Frost Law Group"),
    ...
    ("/practice-areas/rideshare-accidents (new)", "Summerville Uber & Lyft Accident Lawyer | Frost Law Group"),
    ("/practice-areas/drunk-driving-accidents (new)", "Hit by a Drunk Driver in Summerville, SC? | Frost Law Group"),
    ("/areas/goose-creek (new)", "Goose Creek Car Accident Lawyer | Frost Law Group"),
    ("/areas/mount-pleasant · /north-charleston · /moncks-corner · /walterboro (new)", "[City] Car Accident Lawyer | Frost Law Group"),
    ("/es/abogado-de-accidentes (new)", "Abogado de Accidentes de Carro en Summerville, SC | Frost Law Group"),
...
add(table([("From", lambda r: f"<code>{esc(r[0])}</code>", ""), ("To (301)", lambda r: f"<code>{esc(r[1])}</code>", "")], [
    ("http://www.frostlawgroupsc.com/… and https://www.frostlawgroupsc.com/…", "https://frostlawgroupsc.com/…"),
    ("https://frostlawgroupsc.com/page/ (trailing slash)", "https://frostlawgroupsc.com/page"),
    ("/probate-2/", "/probate"),
    ("/about-us/?swcfpc=1 (and any ?swcfpc= address)", "/about-us (strip the parameter)"),
    ("/author/adminfrost-lawgroup-com/", "/about-us (or noindex the author archive)"),
    ("/motor-vehicle-personal-injury", "https://www.summervilleaccidentattorney.com/practice-areas/car-accidents (after the two-site decision)"),
]))
...
add("<h3>E. Glossary</h3>")
add("<dl>" + "".join(f"<dt><strong>{t}</strong></dt><dd>{d}</dd>" for t, d in [
    ("Impression", "One time Google showed a page of yours on a results page (including in the map pack via your Business Profile link and in AI answers)."),
    ("Click", "One time a person chose that result."),
    ("Position", "Where the result sat on the page, averaged over all the times it was shown. 1–10 is page one."),
    ("CTR", "Click-through rate: clicks ÷ impressions."),
    ("Query", "The exact words typed or spoken. Search Console hides very rare queries for privacy, which is why query totals are lower than site totals."),
    ("Brand / non-brand", "Brand searches contain the firm's name (“frost”). Non-brand searches are people who don't know you yet — the only ones that grow a practice."),
    ("Canonical", "The one official address for a page. When a page has several addresses, Google picks one and treats the rest as copies."),
    ("301 redirect", "A permanent forward from an old address to a new one. It sends people and Google's trust to the new page."),
    ("Topical authority", "Being the site Google trusts most on one subject, earned by covering the subject completely and coherently."),
    ("Fan-out", "How Google's AI answers a big question: it splits it into smaller questions, searches each, and stitches the best paragraphs together."),
    ("Structured data (schema)", "Machine-readable labels in the page code that tell Google exactly who you are (LegalService, Attorney), what a block is (FAQPage), and what a video is (VideoObject)."),
    ("E-E-A-T", "Experience, Expertise, Authoritativeness, Trust — the qualities Google looks for on legal and financial pages; shown with real authors, credentials, reviews and consistent facts."),
]) + "</dl>")
add("<h3>F. Method notes</h3>")
add(f"<p class=\"small\">Both properties are domain properties in Google Search Console. The main site has data from {F_FIRST.isoformat()}; the injury site from {S_FIRST.isoformat()}. Query, page and date tables use the property's own reported values; where we group pages by address or queries by topic, the grouping is ours and the underlying numbers are unchanged. Page-level impressions can exceed site-level impressions because one search can show two of your pages. Positions are impression-weighted averages. Observations about the live pages (titles, headings, word counts, not-found pages) were made on September 11, 2026; Business Profile details are as shown publicly on Google the same day.</p>")
add("</section>")
add('<footer class="small" style="margin-top:48px;padding-top:16px;border-top:1px solid var(--hair)">Frost Law Group Search Audit · prepared September 11, 2026 · data: Google Search Console</footer></div>')
...
if ART:
    with open(ART, "w") as f:
        f.write(head + body)
    print("wrote", ART)
print("fan-out questions:", FAN_N, FAN_I, "| quick wins main:", len(quick_frost), "| quick wins injury:", len(quick_sva), "| dead:", len(dead_rows), DEAD_I, "| multi-address pages:", len(MULTI))
